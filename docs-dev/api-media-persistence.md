...
```typescript
// Load project
const projectData = await ProjectFileService.loadProject('/path/to/project.tlsp')
...
describe('MediaRestorationService', () => {
  it('should restore file from original path', async () => {
    ...
    mockValidateFileIntegrity.mockResolvedValue({ isValid: true, confidence: 1.0, issues: [] })
