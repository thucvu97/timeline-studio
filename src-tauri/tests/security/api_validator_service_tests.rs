...
async fn test_service_initialize_with_event_bus() {
    ...
    impl timeline_studio_lib::core::EventHandler for TestHandler {
        ...
        async fn handle(&self, event: Self::Event) -> Result<(), timeline_studio_lib::video_compiler::error::VideoCompilerError> {
...
async fn test_validate_key_with_event_bus() {
    ...
    // Проверяем, что событие было отправлено
    if let Ok(Some(key)) = tokio::time::timeout(
...
async fn test_check_service_availability() {
    ...
    for key_type in services_to_check {
        ...
        assert!(is_available || !is_available); // Просто проверяем, что возвращается bool
    }
}
...
mod concurrent_tests {
    ...
    async fn test_concurrent_validations() {
        ...
        for i in 0..10 {
            ...
            let key_type = if i % 2 == 0 { ApiKeyType::OpenAI } else { ApiKeyType::Claude };
            
            futures.push(tokio::spawn(async move {
                service_clone.validate_key(key_type, &format!("test-key-{}", i)).await
            }));
        }
