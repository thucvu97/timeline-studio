...
fn test_secure_storage_result_with_error() {
    ...
    assert!(!result.encryption_enabled);
    assert_eq!(result.error, Some("Storage creation failed: permission denied".to_string()));
}
...
mod serialization_tests {
    ...
    fn test_all_types_json_roundtrip() {
        ...
        assert_eq!(deserialized.overall_security_score, check_result.overall_security_score);
