...
use timeline_studio_lib::core::plugins::{
    Plugin, PluginMetadata, PluginType, PluginCommand, PluginResponse,
    PluginContext, Version, AppEventType, PluginDependency,
};
...
impl YouTubeUploaderPlugin {
    ...
    async fn start_upload(&self, task: UploadTask) -> Result<()> {
        ...
        if let Some(context) = &self.context {
            context.event_bus.publish_app_event(AppEvent::PluginEvent {
        ...
        loop {
            ...
            if let Some(mut task) = task {
                ...
                for i in 0..=10 {
                    ...
                    if let Some(context) = &self.context {
                        let _ = context.event_bus.publish_app_event(AppEvent::PluginEvent {
                            plugin_id: self.metadata.id.clone(),
                            event: serde_json::json!({
...
impl Plugin for YouTubeUploaderPlugin {
    ...
    async fn initialize(&mut self, context: PluginContext) -> Result<()> {
        ...
        if !context.permissions.network.can_connect("youtube.com", 443) {
            return Err(timeline_studio_lib::video_compiler::error::VideoCompilerError::InvalidParameter(
        ...
        if !queue.is_empty() {
            ...
            if let Some(context) = &self.context {
                ...
                let queue_json = serde_json::to_string_pretty(&*queue)
                    .map_err(|e| timeline_studio_lib::video_compiler::error::VideoCompilerError::SerializationError(e.to_string()))?;
                
                tokio::fs::write(&queue_path, queue_json).await
    ...
    async fn handle_command(&self, command: PluginCommand) -> Result<PluginResponse> {
        ...
        let response = match command.command.as_str() {
            "upload_video" => {
                ...
                let video_path = command.params.get("video_path")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| timeline_studio_lib::video_compiler::error::VideoCompilerError::InvalidParameter(
                        "video_path is required".to_string()
                    ))?;
                
                let title = command.params.get("title")
                    .and_then(|v| v.as_str())
                    .unwrap_or("Untitled Video");
                
                let description = command.params.get("description")
                    .and_then(|v| v.as_str())
                    .unwrap_or("");
            ...
            "get_upload_status" => {
                ...
                let task_id = command.params.get("task_id")
                    .and_then(|v| v.as_str())
                    .unwrap_or("");
            ...
            "set_api_key" => {
                // Установить API ключ
                let api_key = command.params.get("api_key")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| timeline_studio_lib::video_compiler::error::VideoCompilerError::InvalidParameter(
    ...
    async fn handle_event(&self, event: &AppEvent) -> Result<()> {
        match event {
            AppEvent::RenderCompleted { job_id, output_path } => {
