...
use timeline_studio_lib::core::{EventBus, EventHandler, AppEvent};
    ...
    async fn handle(&self, event: Self::Event) -> Result<()> {
        match event {
            AppEvent::ProjectCreated { project_id } => {
                ...
                *count += 1;
                println!("Project created: {}. Total projects: {}", project_id, *count);
            }
    ...
    async fn handle(&self, event: Self::Event) -> Result<()> {
        match event {
            ...
            AppEvent::RenderProgress { job_id, progress } => {
                ...
                println!("📊 Render {}: [{}{}] {:.1}%", job_id, progress_bar, empty_bar, progress * 100.0);
            }
...
async fn main() -> Result<()> {
    ...
    // 1. Создание проекта
    event_bus.publish_app_event(AppEvent::ProjectCreated {
    ...
    // 2. Открытие проекта
    event_bus.publish_app_event(AppEvent::ProjectOpened {
        project_id: "project-001".to_string(),
    ...
    event_bus.publish(CustomMediaEvent {
        media_id: "media-001".to_string(),
    ...
    // 4. Начало рендеринга
    event_bus.publish_app_event(AppEvent::RenderStarted {
    ...
    for i in 0..=10 {
        ...
        event_bus.publish_app_event(AppEvent::RenderProgress {
            job_id: "render-001".to_string(),
            progress,
        }).await?;
    ...
    event_bus.publish_app_event(AppEvent::MemoryWarning {
        usage_percent: 85.5,
    }).await?;
    ...
    // 8. Закрытие проекта
    event_bus.publish_app_event(AppEvent::ProjectClosed {
        project_id: "project-001".to_string(),
    }).await?;
        ...
        async fn handle(&self, event: Self::Event) -> Result<()> {
            match event {
                AppEvent::MediaImported { media_id, .. } => {
                    ...
                    self.event_bus.publish_app_event(AppEvent::RecognitionStarted {
                    ...
                    tokio::spawn({
                        ...
                        async move {
                            ...
                            let _ = event_bus.publish_app_event(AppEvent::RecognitionCompleted {
                                media_id,
