...
use timeline_studio_lib::core::performance::{
    ZeroCopyBuffer, ZeroCopyManager, DataType, VideoZeroCopy, AudioZeroCopy
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let rgb_frame = manager.get_frame_buffer(1920, 1080, DataType::Rgb24).await?;
    println!("  ✅ RGB frame created in {:?} (size: {} bytes)", 
             start.elapsed(), rgb_frame.size());
    ...
    println!("  ✅ RGB->RGBA conversion in {:?} (size: {} bytes)", 
             start.elapsed(), rgba_frame.size());
    ...
    let yuv_frame = manager.get_frame_buffer(1920, 1080, DataType::Yuv420p).await?;
    ...
    println!("  ✅ Extracted YUV planes: Y={} U={} V={} bytes", 
    ...
    let left_channel = manager.get_audio_buffer(samples, DataType::AudioF32).await?;
    let right_channel = manager.get_audio_buffer(samples, DataType::AudioF32).await?;
    
    println!("  Created stereo channels: L={} R={} bytes", 
             left_channel.size(), right_channel.size());
    
    // Интерлив каналов
    let start = Instant::now();
    let interleaved = AudioZeroCopy::interleave_stereo(&left_channel, &right_channel)?;
    println!("  ✅ Interleaved stereo in {:?} (size: {} bytes)", 
             start.elapsed(), interleaved.size());
    ...
    println!("  Total memory: {:.2} MB", stats.total_memory as f64 / 1024.0 / 1024.0);
    println!("  Zero-copy operations: {}", stats.zero_copy_operations);
    println!("  Bytes saved: {:.2} MB", stats.bytes_saved as f64 / 1024.0 / 1024.0);
    
    // Показываем информацию о пулах
    let pools = manager.get_pool_info().await;
    println!("\n🏊 Pool Information:");
    for ((size, data_type), count) in pools {
        println!("  {:?} buffers ({}KB): {} in pool", 
                 data_type, size / 1024, count);
    }
    ...
    println!("  Processing {} frames ({}x{})...", frame_count, width, height);
    let pipeline_start = Instant::now();
    
    for frame_num in 0..frame_count {
        // 1. Декодируем кадр (симуляция)
        let raw_frame = manager.get_frame_buffer(width, height, DataType::Yuv420p).await?;
        
        // 2. Применяем эффект (создаем view)
        let effect_roi = manager.create_view(&raw_frame, 0, raw_frame.size() / 2).await?;
        
        // 3. Конвертируем в RGB для отображения
        let rgb_frame = manager.get_frame_buffer(width, height, DataType::Rgb24).await?;
    ...
    println!("  Memory saved: {:.2} MB", final_stats.bytes_saved as f64 / 1024.0 / 1024.0);
    println!("  Peak memory usage: {:.2} MB", final_stats.total_memory as f64 / 1024.0 / 1024.0);
