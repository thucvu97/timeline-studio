...
pub fn get_media_metadata(file_path: String) -> Result<MediaFile, String> {
    ...
    let output_str = str::from_utf8(&output.stdout)
        .map_err(|e| format!("Ошибка декодирования вывода: {}", e))?;

    // Парсим JSON
    let probe_data: serde_json::Value = serde_json::from_str(output_str)
    ...
    if let (Some(streams_array), Some(format_obj)) = (streams, format) {
        ...
        ffprobe_format.duration = format_obj.get("duration")
            .and_then(|v| v.as_str())
            .and_then(|s| s.parse::<f64>().ok());

        ffprobe_format.size = format_obj.get("size")
            .and_then(|v| v.as_str())
            .and_then(|s| s.parse::<u64>().ok());

        ffprobe_format.bit_rate = format_obj.get("bit_rate")
            .and_then(|v| v.as_str())
            .map(String::from);

        ffprobe_format.format_name = format_obj.get("format_name")
            .and_then(|v| v.as_str())
            .map(String::from);
        ...
        for stream in streams_array {
            ...
            if codec_type == "video" &&
               stream.get("disposition").and_then(|d| d.get("attached_pic")).and_then(|v| v.as_i64()) != Some(1) {
                is_video = true;
        ...
        if let Ok(entry) = entry {
            ...
            if path.is_file() {
                if let Some(extension) = path.extension().and_then(|e| e.to_str()) {
                    ...
                    if ["mp4", "avi", "mkv", "mov", "webm", "mp3", "wav", "ogg", "flac", "jpg", "jpeg", "png", "gif", "webp"]
                        .contains(&ext.as_str()) {
