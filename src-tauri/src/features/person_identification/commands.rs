...
/// Инициализация базы данных персон
#[tauri::command]
pub async fn init_person_database(
    ...
    // Создаем базу данных
    let db = PersonDatabase::new(db_path)
        .await
        .map_err(|e| format!("Failed to create database: {}", e))?;
    
    // Сохраняем в состоянии приложения
    app.manage(PersonDatabaseState(Arc::new(Mutex::new(db))));
    
    Ok("Person database initialized successfully".to_string())
}
...
pub async fn create_person(
    ...
    // Создаем персону
    let mut person = db.create_person(name, description)
        .await
