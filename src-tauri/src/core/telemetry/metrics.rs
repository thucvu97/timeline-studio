...
use opentelemetry::{
    global,
    metrics::{Counter as OtelCounter, Histogram as OtelHistogram, UpDownCounter, Meter},
    KeyValue,
};
        ...
        if let Some(existing_type) = metrics.get(name) {
            if !matches!(
                ...
                return Err(VideoCompilerError::InvalidParameter(
                    format!("Metric '{}' already registered with different type", name)
                ));
            }
        } else {
...
impl Metrics {
    ...
    pub fn new(collector: &MetricsCollector) -> Result<Self> {
        Ok(Self {
            ...
            http_request_size: collector.histogram(
                "http_request_size_bytes",
                "HTTP request size in bytes"
            )?,
            http_response_size: collector.histogram(
                "http_response_size_bytes",
                "HTTP response size in bytes"
            )?,
            http_active_requests: collector.gauge(
                "http_active_requests",
                "Number of active HTTP requests"
            )?,
            
            // Рендеринг метрики
            render_jobs_total: collector.counter(
                "render_jobs_total",
                "Total number of render jobs"
            )?,
            render_jobs_active: collector.gauge(
                "render_jobs_active",
                "Number of active render jobs"
            )?,
            render_duration: collector.histogram(
            ...
            render_errors_total: collector.counter(
                "render_errors_total",
            ...
            media_cache_hits: collector.counter(
                "media_cache_hits_total",
                "Total number of media cache hits"
            )?,
            ...
            // Плагин метрики
            plugin_loads_total: collector.counter(
                "plugin_loads_total",
                "Total number of plugin loads"
            )?,
            plugin_errors_total: collector.counter(
                "plugin_errors_total",
            ...
            plugin_active_count: collector.gauge(
                "plugin_active_count",
...
mod tests {
    ...
    async fn test_metrics_collector_creation() {
        ...
        let histogram = collector.histogram("test_histogram", "Test histogram").unwrap();
