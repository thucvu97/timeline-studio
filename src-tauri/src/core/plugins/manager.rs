        ...
        // Проверяем не загружен ли уже
        {
            ...
            if plugins.contains_key(plugin_id) {
                return Err(VideoCompilerError::InvalidParameter(
                    format!("Plugin '{}' is already loaded", plugin_id)
                ));
            }
        }
        ...
        let mut handle = {
            ...
            plugins.remove(plugin_id)
                .ok_or_else(|| VideoCompilerError::InvalidParameter(
                    format!("Plugin '{}' is not loaded", plugin_id)
                ))?
        };
    ...
    pub async fn send_command(&self, plugin_id: &str, command: PluginCommand) -> Result<PluginResponse> {
        ...
        match handle.state {
            ...
            PluginState::Suspended => {
                return Err(VideoCompilerError::InvalidParameter(
                    format!("Plugin '{}' is suspended", plugin_id)
                ));
            }
            _ => {
                return Err(VideoCompilerError::InvalidParameter(
...
mod tests {
    ...
    async fn test_plugin_manager_lifecycle() {
        ...
        registry.register(super::loader::PluginRegistration {
            metadata,
            factory,
        }).await.unwrap();
        ...
        let instance_id = manager.load_plugin("test-plugin", permissions).await.unwrap();
