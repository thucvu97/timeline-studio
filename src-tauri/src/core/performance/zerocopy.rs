...
impl ZeroCopyBuffer {
    ...
    pub fn new(size: usize, alignment: usize, data_type: DataType) -> Result<Self> {
        if size == 0 {
            return Err(VideoCompilerError::InvalidParameter("Buffer size cannot be zero".to_string()));
        }
        ...
        let ptr = unsafe {
            ...
            if raw_ptr.is_null() {
                return Err(VideoCompilerError::InternalError("Failed to allocate memory".to_string()));
            }
    ...
    pub fn for_audio(samples: usize, data_type: DataType) -> Result<Self> {
        if !matches!(data_type, DataType::AudioF32 | DataType::AudioPcm16) {
            return Err(VideoCompilerError::InvalidParameter("Invalid audio data type".to_string()));
        }
    ...
    pub fn view(&self, offset: usize, len: usize) -> Result<ZeroCopyView> {
        if offset + len > self.size {
            return Err(VideoCompilerError::InvalidParameter("View exceeds buffer bounds".to_string()));
        }
...
impl ZeroCopyManager {
    ...
    pub async fn get_frame_buffer(&self, width: usize, height: usize, data_type: DataType) -> Result<ZeroCopyBuffer> {
        let size = data_type.frame_size(width, height);
    ...
    pub async fn get_audio_buffer(&self, samples: usize, data_type: DataType) -> Result<ZeroCopyBuffer> {
        if !matches!(data_type, DataType::AudioF32 | DataType::AudioPcm16) {
            return Err(VideoCompilerError::InvalidParameter("Invalid audio data type".to_string()));
        }
    ...
    pub async fn transfer_data(&self, src: &ZeroCopyRef, dst: &mut ZeroCopyBuffer) -> Result<()> {
        if src.size() != dst.size() {
            return Err(VideoCompilerError::InvalidParameter("Buffer sizes must match".to_string()));
        }
        
        if src.data_type() != dst.data_type() {
            return Err(VideoCompilerError::InvalidParameter("Data types must match".to_string()));
        }
    ...
    pub async fn create_view(&self, buffer: &ZeroCopyBuffer, offset: usize, len: usize) -> Result<ZeroCopyView> {
...
impl VideoZeroCopy {
    ...
    pub fn rgb_to_rgba_inplace(rgb_buffer: &ZeroCopyBuffer) -> Result<ZeroCopyBuffer> {
        if rgb_buffer.data_type() != DataType::Rgb24 {
            return Err(VideoCompilerError::InvalidParameter("Expected RGB24 buffer".to_string()));
        }
        
        let (width, height) = rgb_buffer.dimensions()
            .ok_or_else(|| VideoCompilerError::InvalidParameter("Buffer missing dimensions".to_string()))?;
    ...
    pub fn extract_yuv_planes(yuv_buffer: &ZeroCopyBuffer) -> Result<(ZeroCopyView, ZeroCopyView, ZeroCopyView)> {
        if yuv_buffer.data_type() != DataType::Yuv420p {
            return Err(VideoCompilerError::InvalidParameter("Expected YUV420p buffer".to_string()));
        }
        
        let (width, height) = yuv_buffer.dimensions()
            .ok_or_else(|| VideoCompilerError::InvalidParameter("Buffer missing dimensions".to_string()))?;
...
impl AudioZeroCopy {
    ...
    pub fn interleave_stereo(left: &ZeroCopyBuffer, right: &ZeroCopyBuffer) -> Result<ZeroCopyBuffer> {
        if left.data_type() != right.data_type() {
            return Err(VideoCompilerError::InvalidParameter("Audio buffers must have same type".to_string()));
        }
        
        if left.size() != right.size() {
            return Err(VideoCompilerError::InvalidParameter("Audio buffers must have same size".to_string()));
        }
