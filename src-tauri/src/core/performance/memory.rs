...
impl MemoryBlock {
    ...
    fn new(size: usize) -> Result<Self> {
        ...
        if ptr.is_null() {
            return Err(VideoCompilerError::InternalError("Memory allocation failed".to_string()));
        }
...
impl PooledBuffer {
    ...
    pub fn as_slice(&self) -> &[u8] {
        if let Some(block) = &self.block {
            unsafe {
                std::slice::from_raw_parts(block.as_ptr(), self.size.min(block.size()))
            }
        } else {
            &[]
        }
    ...
    pub fn as_mut_slice(&mut self) -> &mut [u8] {
        if let Some(block) = &mut self.block {
            unsafe {
                std::slice::from_raw_parts_mut(block.as_ptr(), self.size.min(block.size()))
            }
        } else {
            &mut []
        }
