...
pub async fn extract_thumbnail_candidates(
    ...
    for i in 0..count {
        ...
        let success = extract_high_quality_frame(&video_path, timestamp, &frame_path, aspect_ratio.as_deref()).await?;
...
pub async fn optimize_image_for_analysis(
    ...
    cmd.args([
        "-i", &input_path,
        "-vf", &format!("scale='min({},iw)':'min({},ih)':force_original_aspect_ratio=decrease", max_dim, max_dim),
...
pub async fn cleanup_extracted_frames(clip_id: String) -> Result<u32, String> {
    ...
    log::info!("Удалено {} временных кадров для клипа {}", removed_count, clip_id);
    Ok(removed_count)
}

// Вспомогательные функции

async fn extract_single_frame(video_path: &str, timestamp: f64, output_path: &Path) -> Result<bool, String> {
    let executor = FFmpegExecutor::new();
    let mut cmd = tokio::process::Command::new("ffmpeg");
...
async fn create_ffmpeg_collage(
    ...
    let grid_filter = format!("{}xstack=inputs={}:layout=", 
        filter_inputs.join(""),
...
async fn get_image_info(image_path: &Path) -> Result<ImageInfo, String> {
    ...
    let probe_data: serde_json::Value = serde_json::from_str(&result.stdout)
        .map_err(|_| "Ошибка парсинга ffprobe JSON")?;
...
fn parse_grid_size(grid_str: &str) -> Result<(u32, u32), String> {
    ...
    if parts.len() != 2 {
        return Err(format!("Неверный формат сетки: {}. Ожидается 'WxH'", grid_str));
    }

    let cols: u32 = parts[0].parse()
        .map_err(|_| format!("Неверное количество колонок: {}", parts[0]))?;
    let rows: u32 = parts[1].parse()
