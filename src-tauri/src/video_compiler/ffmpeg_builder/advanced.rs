...
mod tests {
  ...
  #[tokio::test]
  async fn test_thumbnails_command() {
    ...
    assert!(args.iter().any(|s| s.contains("scale=320:180")));
  }

  #[tokio::test]
  async fn test_gif_preview_command() {
    let project = ProjectSchema::new("test".to_string());
    let builder = FFmpegBuilder::new(project);
    ...
    assert!(args.iter().any(|s| s.contains("palettegen")));
  }
}
