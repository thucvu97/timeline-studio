...
fn create_custom_project(format: OutputFormat, quality: u32) -> crate::video_compiler::schema::ProjectSchema {
...
mod advanced_encoding_tests {
    ...
    fn test_advanced_encoding_webm_no_h264_settings() {
        ...
        let has_h264_profile = args.iter().any(|arg| arg == "high") && args.contains(&"-profile:v".to_string());
...
mod metadata_tests {
    ...
    fn test_metadata_with_special_characters() {
        ...
        let has_title = args.iter().any(|arg| arg.contains("测试") && arg.contains("🎬"));
        assert!(has_title);
...
mod performance_tests {
    ...
    fn test_bitrate_calculation_performance() {
        ...
        assert!(duration.as_millis() < 100, "Bitrate calculation too slow: {:?}", duration);
    }
}