...
use crate::video_compiler::commands::video_analysis::{MotionAnalysisResult, CameraMovement, MotionPoint};
...
async fn analyze_camera_movement(
    file_path: &Path,
    sensitivity: f64,
    ...
    let transforms = if temp_file.exists() {
        let content = tokio::fs::read_to_string(&temp_file).await.unwrap_or_default();
...
async fn analyze_motion_profile(
    file_path: &Path,
    sensitivity: f64,
) -> Result<Vec<MotionPoint>> {
...
fn parse_camera_transforms(transforms: &str) -> (f64, f64, f64) {
    ...
    if frame_count > 0 {
        ...
        let panning = (avg_x / 10.0).min(1.0);
        let tilting = (avg_y / 10.0).min(1.0);
        let zooming = (avg_zoom * 10.0).min(1.0);
...
fn calculate_average_intensity(profile: &[MotionPoint]) -> f64 {
    ...
    (sum / profile.len() as f64).min(1.0)
}
...
fn calculate_object_movement(total_motion: f64, camera_movement: &CameraMovement) -> f64 {
    ...
    let camera_motion = (camera_movement.panning + camera_movement.tilting + camera_movement.zooming) / 3.0;
    (total_motion - camera_motion * 0.5).max(0.0).min(1.0)
}
...
mod tests {
    ...
    fn test_calculate_average_intensity() {
        let profile = vec![
            MotionPoint { timestamp: 0.0, intensity: 0.5 },
            MotionPoint { timestamp: 1.0, intensity: 0.7 },
            MotionPoint { timestamp: 2.0, intensity: 0.3 },
        ];
    ...
    fn test_parse_camera_transforms() {
        let transforms = "Frame 1 Transform x:1.5 y:0.8 a:0.01 z:1.05\nFrame 2 Transform x:-1.2 y:-0.5 a:-0.01 z:0.98";
        let (panning, tilting, zooming) = parse_camera_transforms(transforms);
