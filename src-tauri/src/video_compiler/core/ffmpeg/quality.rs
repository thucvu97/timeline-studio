...
async fn analyze_brightness_contrast(file_path: &Path, sample_rate: f64) -> Result<(f64, f64)> {
    ...
    let output = FFmpegCommand::ffmpeg()
        .args(vec![
            ...
            "-vf", &format!("fps={},signalstats", sample_rate),
            "-f", "null",
            "-"
        ])
        .execute()
...
async fn analyze_sharpness(file_path: &Path, sample_rate: f64) -> Result<f64> {
    ...
    let output = FFmpegCommand::ffmpeg()
        .args(vec![
            "-i", &file_path.to_string_lossy(),
            "-vf", &format!("fps={},edgedetect=mode=canny:low=0.1:high=0.4,metadata=print", sample_rate),
            "-f", "null",
            "-"
        ])
        .execute()
...
async fn analyze_saturation(file_path: &Path, sample_rate: f64) -> Result<f64> {
    ...
    let output = FFmpegCommand::ffmpeg()
        .args(vec![
            "-i", &file_path.to_string_lossy(),
            "-vf", &format!("fps={},signalstats=stat=sat", sample_rate),
            "-f", "null",
            "-"
        ])
        .execute()
...
async fn detect_noise_level(file_path: &Path, sample_rate: f64) -> Result<f64> {
    ...
    let _output = FFmpegCommand::ffmpeg()
        .args(vec![
            "-i", &file_path.to_string_lossy(),
            "-vf", &format!("fps={},noise=alls=20:allf=t,metadata=print", sample_rate),
            "-f", "null",
            "-"
        ])
        .execute()
...
async fn check_video_stability(file_path: &Path, sample_rate: f64) -> Result<f64> {
    ...
    let _output = FFmpegCommand::ffmpeg()
        .args(vec![
            "-i", &file_path.to_string_lossy(),
            "-vf", &format!("fps={},vidstabdetect=result={}", sample_rate, temp_file.to_string_lossy()),
            "-f", "null",
            "-"
        ])
        .execute()
    ...
    if temp_file.exists() {
        let transforms = tokio::fs::read_to_string(&temp_file).await.unwrap_or_default();
        ...
        if frame_count > 0 {
            ...
            Ok((1.0 - (avg_movement / 100.0).min(1.0)).max(0.0))
        } else {
...
fn calculate_overall_quality(metrics: &QualityMetrics) -> f64 {
    ...
    let weights = [
        ...
        (metrics.brightness.max(0.3).min(0.7) * 2.5 - 0.75, 0.15), // Яркость - 15%
        (metrics.contrast.max(0.3), 0.15),       // Контраст - 15%
        (metrics.saturation.max(0.2).min(0.8) * 1.67 - 0.33, 0.10), // Насыщенность - 10%
    ...
    if total_weight > 0.0 {
        (total / total_weight).max(0.0).min(1.0)
    } else {
        0.5
    }
}
