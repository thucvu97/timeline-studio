...
fn test_clip_with_custom_properties() {
  ...
  assert_eq!(
    deserialized.color_correction.as_ref().unwrap().brightness,
    0.2
  );
  assert_eq!(deserialized.transform.as_ref().unwrap().rotation, 45.0);
  assert_eq!(
    deserialized.properties.notes,
    Some("Important scene".to_string())
  );
  assert_eq!(deserialized.properties.tags.len(), 2);
  assert_eq!(
    deserialized.properties.custom_metadata.get("rating"),
