...
test.describe("Загрузка проекта с медиафайлами", () => {
  test("создаем и загружаем тестовый проект", async ({ page }) => {
    ...
    const hasProjectButtons =
      (await page
        .locator("button")
        .filter({ hasText: /open|load|открыть/i })
        .count()) > 0 || (await page.locator('[class*="open"], [class*="load"]').count()) > 0
    ...
    if (hasProjectButtons) {
      ...
      if (await fileMenu.isVisible()) {
        ...
        const hasMenuItems = (await page.locator('[role="menuitem"], [class*="menu"]').count()) > 0
  ...
  test("использование Tauri команд для загрузки проекта", async ({ page }) => {
    ...
    const hasDialog = (await page.locator('[role="dialog"], [class*="dialog"]').count()) > 0
  ...
  test("прямое обновление состояния через провайдеры", async ({ page }) => {
    ...
    const appStructure = await page.evaluate(() => {
      const root =
        document.getElementById("__next") || document.querySelector("#root") || document.querySelector("body > div")
      return {
        hasRoot: !!root,
