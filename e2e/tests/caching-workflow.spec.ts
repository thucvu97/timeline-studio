...
test.describe("Caching Workflow E2E Tests", () => {
    ...
    await test.step("Генерация и кэширование превью", async () => {
      ...
      await page.evaluate(() => {
        window.dispatchEvent(
          new CustomEvent("test-media-loaded", {
            detail: {
              files: [{ path: "public/t1.mp4", type: "video", duration: 10.0 }],
            },
          }),
        )
      })
    ...
    await test.step("Открытие настроек кэша", async () => {
      ...
      if (await settingsButton.isVisible()) {
        ...
        if (cacheSection) {
          ...
          const cacheSizeInput = page
            .locator('input[name*="cache"], input[type="number"]')
            .filter({
  ...
  test("тестирование кэширования рендера", async ({ page }) => {
    await test.step("Тестирование кэша промежуточных результатов рендеринга", async () => {
      ...
      await page.evaluate(() => {
        window.dispatchEvent(
          new CustomEvent("test-project-setup", {
            detail: {
              duration: 5.0,
              tracks: [
                {
      ...
      if (await exportButton.isVisible()) {
        ...
        if (await startButton.isVisible()) {
          ...
          while (!completed && attempts < 30) {
            const progress = await isAnyVisible(page, ["text=/completed|finished|100%/i", '[class*="completed"]'])
  ...
  test("тестирование кэша метаданных", async ({ page }) => {
    await test.step("Тестирование кэширования метаданных медиа файлов", async () => {
      ...
      await page.evaluate(() => {
        window.dispatchEvent(
          new CustomEvent("test-media-import", {
            detail: {
              files: [
                {
                  path: "public/t1.mp4",
