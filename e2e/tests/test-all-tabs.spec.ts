...
test.describe("All Browser Tabs Navigation", () => {
  ...
  test("should navigate through all browser tabs", async ({ page }) => {
    const tabs = [
      {
        name: "Media",
        selectors: ['button:has-text("Import")', "text=/no media|empty|drag/i", '[class*="drop"]', '[class*="media"]'],
      },
      {
        name: "Effects",
        selectors: ["text=/effect/i", '[class*="effect"]', 'button:has-text("Apply")', '[class*="grid"]'],
      },
