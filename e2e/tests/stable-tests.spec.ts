...
test.describe("Stable E2E Tests", () => {
  ...
  test("04. Browser tabs functionality", async ({ page }) => {
    ...
    if (tabs > 0) {
      ...
      if (await secondTab.isVisible()) {
        ...
        const secondTabState =
          (await secondTab.getAttribute("aria-selected")) || (await secondTab.getAttribute("data-state"))
  ...
  test("05. Timeline component exists", async ({ page }) => {
    ...
    const timelineSelectors = ['[class*="timeline"]', '[data-testid*="timeline"]', ".timeline", "#timeline"]
  ...
  test("06. Video player area exists", async ({ page }) => {
    ...
    const playerSelectors = ["video", "canvas", '[class*="player"]', '[class*="video"]', '[data-testid*="player"]']
