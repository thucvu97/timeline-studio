...
test.describe('Project Management', () => {
  ...
  test('should create a new project', async ({ page }) => {
    ...
    const hasNewProjectButton = 
      await page.locator('button').filter({ hasText: /new project|create project|new|создать/i }).count() > 0 ||
      await page.locator('[class*="new"], [class*="create"]').count() > 0;
    
    if (hasNewProjectButton) {
      const newProjectButton = page.locator('button').filter({ hasText: /new|create|создать/i }).first();
  ...
  test('should load existing project', async ({ page }) => {
    ...
    const hasOpenDialog = 
      await page.locator('[role="dialog"], [class*="open"], [class*="load"]').count() > 0;
  ...
  test('should show recent projects', async ({ page }) => {
    ...
    const hasRecentButton = 
      await page.locator('button, [role="menuitem"]').filter({ hasText: /recent|история|недавние/i }).count() > 0;
    
    if (hasRecentButton) {
      const recentButton = page.locator('button, [role="menuitem"]').filter({ hasText: /recent|история/i }).first();
      if (await recentButton.isVisible()) {
        ...
        const hasList = 
          await page.locator('[role="menu"], [class*="recent"], [class*="list"]').count() > 0;
  ...
  test('should show project settings', async ({ page }) => {
    ...
    const hasSettingsButton = 
      await page.locator('button').filter({ hasText: /project settings|settings|настройки проекта/i }).count() > 0 ||
      await page.locator('[class*="settings"], button:has-text("⚙")').count() > 0;
    ...
    if (hasSettingsButton) {
      ...
      if (await settingsButton.isVisible()) {
        ...
        if (hasModal) {
          ...
          const closeButton = page.locator('button').filter({ hasText: /close|cancel|×|закрыть/i }).first();
  ...
  test('should handle project export', async ({ page }) => {
    ...
    const hasExportButton = 
      await page.locator('button').filter({ hasText: /export|render|экспорт/i }).count() > 0 ||
      await page.locator('[class*="export"], [class*="render"]').count() > 0;
    
    if (hasExportButton) {
      const exportButton = page.locator('button').filter({ hasText: /export|render|экспорт/i }).first();
      if (await exportButton.isVisible()) {
        ...
        if (hasExportDialog) {
          // Закрываем диалог
          const closeButton = page.locator('button').filter({ hasText: /cancel|close|×|отмена/i }).first();
          if (await closeButton.isVisible()) {
            await closeButton.click();
          } else {
            await page.keyboard.press('Escape');
