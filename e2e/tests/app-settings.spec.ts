...
test.describe('Application Settings', () => {
  ...
  test('should open settings dialog', async ({ page }) => {
    const settingsButton = page.locator('button[aria-label*="settings"], button[aria-label*="preferences"], [class*="settings-button"]').first();
  ...
  test('should show keyboard shortcuts', async ({ page }) => {
    ...
    if (await settingsButton.isVisible()) {
      ...
      if (shortcutsSection) {
        const shortcutsButton = page.locator('button').filter({ hasText: /shortcuts/i }).first();
  ...
  test('should save settings', async ({ page }) => {
    ...
    if (await settingsButton.isVisible()) {
      ...
      // Save settings
      const saveButton = page.locator('button').filter({ hasText: /save|apply|ok/i }).first();
