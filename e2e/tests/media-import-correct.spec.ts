...
test.describe('Media Import - Correct Implementation', () => {
  ...
  test('should import media files with progressive loading', async ({ page }) => {
    ...
    const hasImportButton = 
      await page.locator('button').filter({ hasText: /import|add|upload|browse/i }).count() > 0 ||
      await page.locator('[class*="import"], [class*="add"]').count() > 0;
  ...
  test('should show media file details after import', async ({ page }) => {
    ...
    const hasMediaItems = 
      await page.locator('[class*="media"][class*="item"], [class*="file"], img[src]').count() > 0;
    ...
    if (hasMediaItems) {
      ...
      const hasEmptyState = 
        await page.locator('text=/no media|empty|import|drag/i').count() > 0;
  ...
  test('should handle video registration for streaming', async ({ page }) => {
    ...
    if (hasVideoElements) {
      ...
      if (await videoItem.isVisible()) {
        ...
        const hasPlayerOrChange = 
          await page.locator('video[src], [class*="playing"], [class*="active"]').count() > 0;
