...
test.describe("Music Browser", () => {
  ...
  test("should support music search", async ({ page }) => {
    ...
    if (await searchInput.isVisible()) {
      ...
      const hasFilteredResults =
        (await page.locator("text=/ambient/i").count()) > 0 || (await page.locator('[class*="result"]').count()) > 0
  ...
  test("should support loop mode", async ({ page }) => {
    ...
    if ((await loopButton.count()) > 0) {
      ...
      const hasLoopIndicator = (await page.locator('[class*="loop"], [class*="repeat"]').count()) > 0
