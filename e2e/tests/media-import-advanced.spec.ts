...
test.describe("Advanced Media Import Features", () => {
  ...
  test("should handle folder import with progress tracking", async ({ page }) => {
    ...
    const hasFolderButton =
      (await page
        .locator("button")
        .filter({ hasText: /folder|directory|папка/i })
        .count()) > 0
  ...
  test("should update placeholders progressively", async ({ page }) => {
    ...
    if (await importButton.isVisible()) {
      ...
      const hasThumbnails = (await page.locator('img[src], [class*="thumbnail"], [class*="preview"]').count()) > 0
  ...
  test("should handle mixed content with different aspect ratios", async ({ page }) => {
    ...
    const hasViewModes =
      (await page
        .locator("button")
        .filter({ hasText: /grid|list|thumbnail/i })
        .count()) > 0 || (await page.locator('[class*="view"], [class*="mode"]').count()) > 0
    ...
    const hasMediaItems = (await page.locator('[class*="media"][class*="item"], img, video').count()) > 0
  ...
  test("should remember import settings", async ({ page }) => {
    ...
    const hasSettingsButton =
      (await page
        .locator("button")
        .filter({ hasText: /settings|настройки|preferences/i })
        .count()) > 0 || (await page.locator('button:has-text("⚙")').count()) > 0
    ...
    if (hasSettingsButton) {
      ...
      if (await settingsButton.isVisible()) {
        ...
        if (hasSettingsDialog) {
          ...
          const hasOptions = (await page.locator('input[type="checkbox"], select, [role="checkbox"]').count()) > 0

          // Закрываем диалог
          const closeButton = page
            .locator("button")
            .filter({ hasText: /close|save|×/i })
            .first()
  ...
  test("should support keyboard navigation during import", async ({ page }) => {
    // Проверяем наличие медиа элементов
    const hasMediaItems = (await page.locator('[class*="media"][class*="item"], [class*="file"]').count()) > 0
