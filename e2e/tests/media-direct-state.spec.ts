  ...
  test("загружаем медиафайлы через состояние приложения", async ({ page }) => {
    ...
    const stateUpdated = await page.evaluate(() => {
      ...
      const possibleKeys = Object.keys(window).filter(
        (key) => key.includes("actor") || key.includes("machine") || key.includes("state") || key.includes("xstate"),
      )
    ...
    const appState = await page.evaluate(() => {
      const result: any = {
        ...
        windowKeys: Object.keys(window)
          .filter((k) => k.includes("APP") || k.includes("STATE") || k.includes("STORE") || k.includes("SETTINGS"))
