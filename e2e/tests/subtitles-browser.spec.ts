...
test.describe('Subtitles Browser', () => {
  ...
  test('should display subtitle options', async ({ page }) => {
    ...
    const hasOptions = 
      ...
      await page.locator('button').filter({ hasText: /add|create|import/i }).count() > 0;
  ...
  test('should allow manual subtitle creation', async ({ page }) => {
    ...
    const createButton = page.locator('button').filter({ hasText: /create|add.*subtitle/i }).first();
  ...
  test('should support subtitle import', async ({ page }) => {
    // Ищем кнопку импорта
    const importButton = page.locator('button').filter({ hasText: /import|upload.*srt/i }).first();
    ...
    if (await importButton.isVisible()) {
      ...
      const hasImportDialog = 
        await page.locator('[role="dialog"], [class*="import"]').count() > 0;
  ...
  test('should allow subtitle export', async ({ page }) => {
    ...
    const exportButton = page.locator('button').filter({ hasText: /export|download.*srt/i }).first();
