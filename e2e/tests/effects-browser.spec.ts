...
test.describe('Effects Browser', () => {
  ...
  test('should allow effect search', async ({ page }) => {
    ...
    if (await searchInput.isVisible()) {
      ...
      const hasFilteredResults = 
        await page.locator('text=/blur/i').count() > 0;
  ...
  test('should allow drag and drop to timeline', async ({ page }) => {
    ...
    if (await effectItem.count() > 0) {
      ...
      const hasDragIndicator = 
        await page.locator('[class*="dragging"], [class*="drag"]').count() > 0;
  ...
  test('should show empty state when no effects', async ({ page }) => {
    ...
    if (await searchInput.isVisible()) {
      ...
      const hasEmptyState = 
        await page.locator('text=/no.*found|no.*results|empty/i').count() > 0;
  ...
  test('should support effect preview', async ({ page }) => {
    ...
    const previewButton = page.locator('button').filter({ hasText: /preview|try/i }).first();
    ...
    if (await previewButton.isVisible()) {
      ...
      const hasPreview = 
        await page.locator('[class*="preview"], video, canvas').count() > 0;
