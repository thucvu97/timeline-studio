...
1. **Plugin API Full Implementation** (`src/core/plugins/api.rs`)
   ...
   - [ ] Полная реализация `get_media_info()` через MediaProcessor
   - [ ] Реализовать `apply_effect()` через EffectProcessor
   - [ ] Полная реализация `generate_thumbnail()` через PreviewService
   - [ ] Интегрировать операции с timeline через ProjectService
   - [ ] Реализовать UI методы через Tauri команды
   - [ ] Интегрировать с EventBus для публикации событий
