...
1. **Plugin API Implementation** (`src/core/plugins/api.rs`)
   - [ ] Реализовать `get_media_info()`
   - [ ] Реализовать `apply_effect()`
   - [ ] Реализовать `generate_thumbnail()`
   - [ ] Реализовать операции с timeline (add/remove/update clip)
   - [ ] Реализовать UI методы (dialog, menu)
   - [ ] Реализовать файловые операции (read/write)
   - [ ] Реализовать storage API
