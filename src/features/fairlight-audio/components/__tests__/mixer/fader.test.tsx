...
describe("Fader", () => {
  ...
  describe("rendering", () => {
    it("renders solo and mute buttons", () => {
      const { getByText } = renderWithAppProvider(<Fader {...defaultProps} />)
      expect(getByText("S")).toBeInTheDocument()
      expect(getByText("M")).toBeInTheDocument()
    })

    it("renders fader handle at correct position", () => {
      const { container } = renderWithAppProvider(<Fader {...defaultProps} value={50} />)
    ...
    it("renders value track at correct height", () => {
      const { container } = renderWithAppProvider(<Fader {...defaultProps} value={75} />)
    ...
    it("displays dB value when dbScale is true", () => {
      const { getByText } = renderWithAppProvider(<Fader {...defaultProps} value={100} dbScale={true} />)
      expect(getByText("0.0 dB")).toBeInTheDocument()
    })

    it("displays percentage value when dbScale is false", () => {
      const { getByText } = renderWithAppProvider(<Fader {...defaultProps} value={75} dbScale={false} />)
      expect(getByText("75%")).toBeInTheDocument()
    })

    it("displays -∞ for 0 value in dB scale", () => {
      const { getByText } = renderWithAppProvider(<Fader {...defaultProps} value={0} dbScale={true} />)
      expect(getByText("-∞ dB")).toBeInTheDocument()
    })

    it("renders dB scale markers", () => {
      const { getByText } = renderWithAppProvider(<Fader {...defaultProps} dbScale={true} />)
    ...
    it("renders label when provided", () => {
      const { getByText } = renderWithAppProvider(<Fader {...defaultProps} label="Master" />)
      expect(getByText("Master")).toBeInTheDocument()
    })

    it("applies custom className", () => {
      const { container } = renderWithAppProvider(<Fader {...defaultProps} className="custom-class" />)
  ...
  describe("interactions", () => {
    ...
    it("updates value when dragging fader", () => {
      const { container } = renderWithAppProvider(<Fader {...defaultProps} />)
  ...
  describe("solo/mute button states", () => {
    it("applies solo active styles", () => {
      const { getByText } = renderWithAppProvider(<Fader {...defaultProps} solo={true} />)
      const soloButton = getByText("S")
    ...
    it("applies solo inactive styles", () => {
      const { getByText } = renderWithAppProvider(<Fader {...defaultProps} solo={false} />)
      const soloButton = getByText("S")
    ...
    it("applies mute active styles", () => {
      const { getByText } = renderWithAppProvider(<Fader {...defaultProps} muted={true} />)
      const muteButton = getByText("M")
    ...
    it("applies mute inactive styles", () => {
      const { getByText } = renderWithAppProvider(<Fader {...defaultProps} muted={false} />)
      const muteButton = getByText("M")
  ...
  describe("dB conversion", () => {
    ...
    it("converts 50% to -6.0 dB", () => {
      const { getByText } = renderWithAppProvider(<Fader {...defaultProps} value={50} dbScale={true} />)
      expect(getByText("-6.0 dB")).toBeInTheDocument()
    })

    it("converts 25% to -12.0 dB", () => {
      const { getByText } = renderWithAppProvider(<Fader {...defaultProps} value={25} dbScale={true} />)
  ...
  describe("automation integration", () => {
    ...
    it("renders with automation props when channelId provided", () => {
      ...
      const { container } = renderWithAppProvider(<Fader {...defaultProps} channelId="ch1" parameterId="volume" />)
      expect(container).toBeInTheDocument()
