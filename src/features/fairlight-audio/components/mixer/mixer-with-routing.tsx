    ...
    <div className={`flex flex-col h-full bg-zinc-950 ${className}`}>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
          ...
          <div className="flex h-full">
            ...
            {groups.length > 0 && (
              <div className="w-auto bg-zinc-800 border-l border-zinc-700 p-2">
                <div className="text-xs text-zinc-400 mb-2 text-center">Groups</div>
        ...
        <TabsContent value="groups" className="flex-1 m-0 p-4">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-zinc-200">Channel Groups</h2>
            ...
            {groups.length === 0 ? (
              <div className="text-center py-8 text-zinc-500">
                ...
                <div className="text-lg mb-2">No groups created</div>
                <div className="text-sm">Groups help organize and control multiple channels together</div>
                ...
                {groups.map((group) => (
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <div className="text-sm text-zinc-400">Channels: {group.channelIds.length}</div>
                      <div className="text-xs text-zinc-500">{group.channelIds.join(", ")}</div>
                      <div className="text-xs text-zinc-500">Bus: {group.busId.replace("_bus", "")}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <TabsContent value="sends" className="flex-1 m-0 p-4">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-zinc-200">Send Matrix</h2>
                  ...
                  return (
                    ...
                    >
                      <div className="text-left">
                        ...
                        <div className="text-xs opacity-70">{channelSends.length} sends</div>
                      </div>
                    </Button>
                  )
                })}
                ...
                {selectedChannelForSends ? (
                  ...
                  <div className="flex items-center justify-center h-64 text-zinc-500">
                    <div className="text-center">
                      ...
                      <div>Select a channel to manage sends</div>
                    </div>
                  </div>
                )}
              </div>
