...
class WienerFilterProcessor extends AudioWorkletProcessor {
  ...
  applyWienerFilter(spectrum, noiseFloor, strength, smoothing, adaptationRate) {
    const noiseFloorLinear = 10 ** (noiseFloor / 20)
    this.frameCount++
    ...
    for (let i = 0; i < this.fftSize / 2; i++) {
      ...
      if (this.frameCount < 10 || magnitude < this.signalSpectrum[i] * 0.5) {
        ...
        this.noiseSpectrum[i] = this.noiseSpectrum[i] * (1 - adaptationRate) + magnitude * adaptationRate
      }

      // Update signal spectrum with smoothing
      this.signalSpectrum[i] = this.signalSpectrum[i] * this.smoothingFactor + magnitude * (1 - this.smoothingFactor)
