...
vi.mock("../components/template-preview", () => ({
  TemplatePreview: ({ template }: any) => (
    <div data-testid={`template-preview-${template.id}`}>
      ...
      <div data-testid={`template-screens-${template.screens}`}>Screens: {template.screens}</div>
      <div data-testid={`template-split-${template.split}`}>Split: {template.split}</div>
      {template.resizable && <div data-testid="template-resizable">Resizable</div>}
    </div>
  ),
}))
