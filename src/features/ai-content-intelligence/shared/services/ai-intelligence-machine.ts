    ...
    const adaptations: AdaptedContent[] = platforms.map((platform) => ({
      ...
      adaptations: {
        ...
        text: {
          ...
          description: { text: "Description", language: "en", characterCount: 11 },
          captions: { enabled: false, style: {} as any, language: "en", content: [] },
          hashtags: [],
          mentions: [],
        },
        graphics: {
...
export const aiIntelligenceMachine = setup({
  ...
  states: {
    ...
    paused: {
      ...
      on: {
        RESUME: [
          { target: "analyzing", guard: ({ context }) => context.currentStep === "content_analysis" },
          { target: "generatingScript", guard: ({ context }) => context.currentStep === "script_generation" },
          { target: "adaptingForPlatforms", guard: ({ context }) => context.currentStep === "platform_adaptation" },
        ],
        CANCEL: "cancelled",
      },
    },
