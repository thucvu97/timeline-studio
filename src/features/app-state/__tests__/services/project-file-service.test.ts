...
import { ProjectFile, ProjectSettings } from "@/features/project-settings/types/project"
...
describe("ProjectFileService", () => {
  ...
  describe("loadProject", () => {
    ...
    it("должен загрузить проект из файла", async () => {
      ...
      const result = await ProjectFileService.loadProject("/path/to/project.tlsp")
    ...
    it("должен обрабатывать ошибки чтения файла", async () => {
      ...
      await expect(ProjectFileService.loadProject("/invalid/path.tlsp")).rejects.toThrow(
        "Failed to load project: Error: File not found",
      )
    })
    ...
    it("должен обрабатывать невалидный JSON", async () => {
      ...
      await expect(ProjectFileService.loadProject("/path/to/project.tlsp")).rejects.toThrow(
        "Failed to load project: SyntaxError",
      )
    })
    ...
    it("должен валидировать структуру проекта", async () => {
      ...
      await expect(ProjectFileService.loadProject("/path/to/project.tlsp")).rejects.toThrow("Invalid project structure")
    })
    ...
    it("должен отклонять неподдерживаемые версии проектов", async () => {
      ...
      await expect(ProjectFileService.loadProject("/path/to/project.tlsp")).rejects.toThrow(
        "Unsupported project version: 0.9.0",
      )
    })
  })
  ...
  describe("saveProject", () => {
    ...
    it("должен сохранить проект в файл", async () => {
      ...
      await ProjectFileService.saveProject("/path/to/project.tlsp", mockProjectData)

      expect(mockWriteTextFile).toHaveBeenCalledWith("/path/to/project.tlsp", expect.any(String))
    })
    ...
    it("должен обрабатывать ошибки записи файла", async () => {
      ...
      await expect(ProjectFileService.saveProject("/readonly/project.tlsp", mockProjectData)).rejects.toThrow(
        "Failed to save project: Error: Permission denied",
      )
    })
  })
  ...
  describe("updateMediaLibrary", () => {
    ...
    it("должен обновить медиабиблиотеку проекта", () => {
      const updatedProject = ProjectFileService.updateMediaLibrary(baseProject, mockMediaFiles, mockMusicFiles)
      ...
      expect(updatedProject.mediaLibrary.lastUpdated).toBeGreaterThan(baseProject.mediaLibrary.lastUpdated)
    })
    ...
    it("должен сохранить остальные данные проекта", () => {
      ...
      expect(updatedProject.projectFavorites).toEqual(baseProject.projectFavorites)
    })
  })
