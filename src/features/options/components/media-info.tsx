...
export function MediaInfo({ selectedMediaFile }: MediaInfoProps) {
  ...
  return (
    <div className="space-y-6" data-testid="media-info">
      <div>
        <h2 className="text-lg font-semibold mb-4">{t("options.info.title", "Информация о медиафайле")}</h2>
          ...
          <>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <InfoRow label={t("options.info.fileName", "Имя файла")} value={selectedFile.name} />
                <InfoRow label={t("options.info.fileSize", "Размер файла")} value={formatFileSize(selectedFile.size)} />
                <InfoRow
                ...
                />
                <InfoRow label={t("options.info.fps", "Частота кадров")} value={`${selectedFile.fps} fps`} />
                <InfoRow label={t("options.info.bitrate", "Битрейт")} value={formatBitrate(selectedFile.bitrate)} />
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <InfoRow label={t("options.info.codec", "Кодек")} value={selectedFile.videoCodec} />
                <InfoRow
                  label={t("options.info.colorSpace", "Цветовое пространство")}
                  value={selectedFile.colorSpace}
                />
                <InfoRow label={t("options.info.profile", "Профиль")} value={selectedFile.profile} />
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <InfoRow label={t("options.info.codec", "Кодек")} value={selectedFile.audioCodec} />
                <InfoRow
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                />
                {selectedFile.device && (
                  <InfoRow label={t("options.info.device", "Устройство")} value={selectedFile.device} />
                )}
              </CardContent>
            </Card>
          </>
