...
import { TemplateListProvider, useTemplateList } from "./template-list-provider"
...
describe("TemplateListProvider", () => {
    ...
    const TestComponent = () => {
      ...
      return (
        <div>
          ...
          <span data-testid="show-favorites">{context.showFavoritesOnly.toString()}</span>
        </div>
      )
    }
  ...
  it("should throw error when useTemplateList is used outside of TemplateListProvider", () => {
    ...
    expect(() => renderHook(() => useTemplateList())).toThrow("useTemplateList must be used within a TemplateListProvider")
