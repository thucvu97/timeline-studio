...
/**
 * Компонент превью для эффектов (унаследован из старого адаптера)
 */
...
/**
 * Новый хук для создания адаптера эффектов с использованием EffectsProvider
 * Совместим с существующим интерфейсом ListAdapter
 */
export function useEffectsAdapterNew(): ListAdapter<VideoEffect> {
  // Используем новый унифицированный адаптер
  const resourcesAdapter = useResourcesAdapter("effects", {})
  const { isItemFavorite } = useFavorites()

  return {
    // Хук для получения данных - адаптируем новый интерфейс к старому
    useData: () => ({
      items: resourcesAdapter.items as VideoEffect[],
      loading: resourcesAdapter.loading,
      error: resourcesAdapter.error,
    }),

    // Компонент превью остается тем же
