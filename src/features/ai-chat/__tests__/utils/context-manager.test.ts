...
describe("context-manager", () => {
  ...
  describe("trimMessagesForContext", () => {
    it("должен сохранять сообщения, если они помещаются в контекст", () => {
      const messages = [
        createTestMessage("Short message 1"),
        createTestMessage("Short message 2"),
      ]
    ...
    it("должен обрезать старые сообщения, если контекст переполнен", () => {
      const messages = [
        createTestMessage("Old message"),
        createTestMessage("Recent message"),
      ]
  ...
  describe("isContextOverLimit", () => {
    it("должен определять, превышает ли контекст лимиты", () => {
      const shortMessages = [
        createTestMessage("Short"),
        createTestMessage("Messages"),
      ]
      
      expect(isContextOverLimit(shortMessages, "claude-4-sonnet")).toBe(false)
      
      const longMessages = Array(10000).fill(null).map((_, i) => 
        createTestMessage(`Very long message number ${i} with lots of content and text that should exceed context limits`)
      )
  ...
  describe("compressContext", () => {
    ...
    it("должен использовать обычную обрезку для малого количества сообщений", () => {
      const messages = Array(5).fill(null).map((_, i) => 
        createTestMessage(`Very long message ${i + 1} `.repeat(1000))
      )
