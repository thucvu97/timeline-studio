            ...
            clips:
              ...
              currentProject.sections.reduce(
                (sum: number, section: TimelineSection) =>
                  sum + section.tracks.reduce((s: number, track: TimelineTrack) => s + track.clips.length, 0),
                0,
              ),
...
function calculateProjectComplexity(project: TimelineProject): number {
  ...
  const trackCount =
    ...
    project.sections.reduce((sum: number, section: TimelineSection) => sum + section.tracks.length, 0)
...
function estimateRenderTime(project: TimelineProject): number {
  ...
  const totalDuration = Math.max(
    ...project.globalTracks.map((track: TimelineTrack) =>
      Math.max(...track.clips.map((clip: TimelineClip) => clip.startTime + clip.duration), 0),
    ),
