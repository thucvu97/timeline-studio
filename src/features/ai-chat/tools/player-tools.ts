...
async function analyzeCurrentMedia(input: Record<string, any>): Promise<PlayerToolResult> {
  ...
  if (includeMetadata && currentMedia.probeData) {
    const videoStream = currentMedia.probeData.streams?.find((s) => s.codec_type === "video")
...
async function loadTemplateFromResources(templateId: string): Promise<any> {
  ...
  try {
    ...
    const templatesModule = await import("../../templates/lib/all-template-configs")
    const template = templatesModule.ALL_TEMPLATE_CONFIGS.find((t: any) => t.id === templateId)
