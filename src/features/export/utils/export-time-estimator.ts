...
export function estimateExportTime(settings: ExportSettings, projectMetrics: ProjectMetrics): TimeEstimate {
  const factors: string[] = []
  let confidence: "low" | "medium" | "high" = "medium"

  // Определяем базовую скорость кодирования
  const codecKey = getCodecKey(settings.format, settings.quality)
  const baseRatio = BASE_RATIOS[codecKey] || 1.0
  ...
  // Клипы
  const clipMultiplier = 1 + projectMetrics.clipCount * COMPLEXITY_MULTIPLIERS.clip
  ...
  if (projectMetrics.effectsCount > 0) {
    const effectMultiplier = 1 + projectMetrics.effectsCount * COMPLEXITY_MULTIPLIERS.effect
  ...
  if (projectMetrics.hasComplexEffects) {
    const complexEffectMultiplier = 1 + COMPLEXITY_MULTIPLIERS.complexEffect
    complexityMultiplier *= complexEffectMultiplier
    factors.push(`Complex effects (+${Math.round(COMPLEXITY_MULTIPLIERS.complexEffect * 100)}%)`)
...
function getCodecKey(format: string, quality: string): keyof typeof BASE_RATIOS {
 ...
 */
export function getConfidenceIcon(confidence: "low" | "medium" | "high"): string {
  switch (confidence) {
    case "high":
      return "✓"
    case "medium":
