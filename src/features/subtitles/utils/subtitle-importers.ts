...
export function importFromVTT(content: string): SubtitleClip[] {
  ...
  for (const block of blocks) {
    ...
    const positionMatch = timeLine.match(/position:(\d+)%/)
    if (positionMatch) {
      position = {
        x: parseInt(positionMatch[1]) / 100,
...
function parseASSColor(assColor: string): string {
  ...
  const match = assColor.match(/&H([A-F0-9]{2})?([A-F0-9]{2})([A-F0-9]{2})([A-F0-9]{2})&/i)
...
function parseASSStyles(content: string): Record<string, SubtitleClip['style']> {
  ...
  // Ищем секцию стилей
  const stylesMatch = content.match(/\[V4\+ Styles\]([\s\S]*?)(?:\[|$)/i)
  ...
  for (const line of lines) {
    ...
    const styleParts = line.substring(6).split(',').map(p => p.trim())
    ...
    if (fontSizeIndex >= 0) {
      style.fontSize = parseInt(styleParts[fontSizeIndex])
    }
    ...
    if (alignmentIndex >= 0) {
      const alignment = parseInt(styleParts[alignmentIndex])
  ...
  for (let i = 0; i < subtitles.length; i++) {
    ...
    if (i > 0) {
      ...
      if (subtitle.startTime < prevEnd) {
        errors.push(
          `Субтитр ${i + 1}: перекрывается с предыдущим субтитром`
        )
      }
    }
  }
