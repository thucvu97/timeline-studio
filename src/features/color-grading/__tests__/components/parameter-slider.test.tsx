...
describe("ParameterSlider", () => {
  ...
  it("should reset to default value on double click", async () => {
    ...
    const { container } = render(
      <ParameterSlider {...defaultProps} defaultValue={25} />
    )
  ...
  it("should not reset when disabled", async () => {
    const user = userEvent.setup()
    const { container } = render(
      <ParameterSlider {...defaultProps} defaultValue={25} disabled />
    )
  ...
  it("should show different colors based on value relative to default", () => {
    const { container, rerender } = render(
      <ParameterSlider {...defaultProps} defaultValue={50} />
    )
  ...
  it("should calculate correct percentage for fill width", () => {
    const { container } = render(
      <ParameterSlider {...defaultProps} value={25} min={0} max={100} />
    )
  ...
  it("should not show tooltip when disabled", () => {
    render(
      <ParameterSlider {...defaultProps} defaultValue={50} disabled />
    )
  ...
  it("should apply custom className", () => {
    const { container } = render(
      <ParameterSlider {...defaultProps} className="custom-class" />
    )
  ...
  it("should handle edge values correctly", () => {
    const { container, rerender } = render(
      <ParameterSlider {...defaultProps} value={0} min={0} max={100} />
    )
  ...
  it("should handle negative ranges", () => {
    const { container } = render(
      <ParameterSlider {...defaultProps} value={0} min={-100} max={100} />
    )
  ...
  it("should position default value mark correctly in negative range", () => {
    const { container } = render(
      <ParameterSlider {...defaultProps} value={25} min={-100} max={100} defaultValue={0} />
    )
