...
export function ColorWheel({ 
  ...
  const rgbToPosition = useCallback((rgb: RGBValue): { x: number; y: number } => {
  ...
  const positionToRgb = useCallback((x: number, y: number): RGBValue => {
  ...
  useEffect(() => {
    ...
    centerGradient.addColorStop(0, `rgba(${type === 'lift' ? '0,0,0' : type === 'gain' ? '255,255,255' : '128,128,128'}, 0.3)`)
  ...
  return (
    <div className="text-center space-y-2">
      ...
      <div className="grid grid-cols-3 gap-1 text-xs">
        <div>R: <span className="text-red-400">{value.r.toFixed(2)}</span></div>
        <div>G: <span className="text-green-400">{value.g.toFixed(2)}</span></div>
        <div>B: <span className="text-blue-400">{value.b.toFixed(2)}</span></div>
      </div>
    </div>
  )
}