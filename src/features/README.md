...
- **Media, Music, Effects, Filters, Subtitles, Transitions, Templates, Styled Templates** - контент для Browser
...
## 🚀 Следующие шаги (обновленный план с Video Compiler)

### 🔥 Критический приоритет - Основа видеоредактора

1. **Создать Video Compiler модуль (Rust/FFmpeg)** - центральная система компиляции видео
   - Добавить `ffmpeg-next`, `uuid`, `tokio` в Cargo.toml
   - Создать структуру модуля `src-tauri/src/video_compiler/`
   - Реализовать `ProjectSchema`, `Timeline`, `Track`, `Clip`, `Effect`
   - Добавить Tauri commands: `compile_video()`, `get_render_progress()`, `generate_preview()`
   - Базовые тесты интеграции с FFmpeg

2. **Завершить Timeline Engine и интеграцию** - UI для управления проектами
   - Доработать Timeline React компонент (машина состояний)
   - Интегрировать с Video Compiler через Tauri commands
   - Управление треками, клипами, эффектами
   - Синхронизация с VideoPlayer

### 🚀 Высокий приоритет - UX и производительность

3. **Preview Generator и Progress Tracking** - пользовательский опыт
   - Генерация превью кадров через FFmpeg
   - WebSocket для отслеживания прогресса рендеринга в реальном времени
   - Кэширование превью для производительности
   - UI компоненты для отображения прогресса и превью

4. **Улучшить покрытие тестами критических модулей**:
   - Templates: с 9.63% до 80%+ (много шаблонов без тестов)
   - Export: с 3.64% до 80%+ (интеграция с Video Compiler)
   - Keyboard Shortcuts: с 2.24% до 80%+ (практически нет тестов)

### 📝 Средний приоритет - Дополнительный функционал

5. **Проверить AI Chat** - убедиться в корректности работы и добавить в отчет покрытия
6. **Расширить Options** - определить и реализовать недостающий функционал, добавить тесты
7. **Оптимизация производительности** - многопоточность, кэширование, память
