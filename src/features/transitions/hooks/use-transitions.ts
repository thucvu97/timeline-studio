...
export function useTransitions(): UseTransitionsReturn {
  ...
  const loadTransitions = useCallback(() => {
    try {
      ...
      console.log(`✅ ${t('transitions.messages.transitionsLoaded', 'Loaded {{count}} transitions from JSON', { count: processedTransitions.length })}`);

    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : t('transitions.errors.unknownError', 'Unknown error');
      setError(t('transitions.errors.failedToLoadTransitions', 'Failed to load transitions: {{error}}', { error: errorMessage }));
      ...
      console.error(`❌ ${t('transitions.errors.fallbackTransitions', 'Failed to load transitions, using fallback')}:`, err);
    } finally {
      setLoading(false);
    }
  }, [t]);
...
export function useTransitionsSearch(query: string, lang: 'ru' | 'en' = 'ru'): Transition[] {
  ...
  return transitions.filter((transition: Transition) =>
