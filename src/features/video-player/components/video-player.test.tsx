...
vi.mock("./player-controls", () => ({
  PlayerControls: ({ currentTime, file }: any) => (
    <div data-testid="player-controls" data-current-time={currentTime} data-file-id={file.id}>
      Player Controls
    </div>
  ),
}))
...
describe("VideoPlayer", () => {
  ...
  it("should render video element with correct source", () => {
    ...
    const videoElement = screen.getByTestId("aspect-ratio").querySelector("video")
    expect(videoElement).not.toBeNull()
    expect(videoElement).toHaveAttribute("src", "converted-/path/to/test-video.mp4")
  })
