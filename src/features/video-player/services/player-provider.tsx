...
export function PlayerProvider({ children }: PlayerProviderProps) {
  ...
  useEffect(() => {
    if (userSettings.playerVolume >= 0) {
      console.log(
        "[PlayerProvider] Initializing volume from user settings:",
        userSettings.playerVolume,
      );
  ...
  const contextValue = {
    ...
    setCurrentTime: (currentTime: number) =>
      send({ type: "setCurrentTime", currentTime }),
    ...
    setIsRecording: (isRecording: boolean) =>
      send({ type: "setIsRecording", isRecording }),
    ...
    setVideoReady: (isReady: boolean) =>
      send({ type: "setVideoReady", isVideoReady: isReady }),
