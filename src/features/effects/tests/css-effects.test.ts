  ...
  describe('generateCSSFilterForEffect', () => {
    ...
    it('should handle complex CSS filters', () => {
      const complexEffect: VideoEffect = {
        ...mockEffect,
        cssFilter: (params) => 
          `blur(${params.radius || 0}px) brightness(${params.brightness || 1}) contrast(${params.contrast || 1})`,
        params: { radius: 3, brightness: 1.2, contrast: 1.1 }
      };
