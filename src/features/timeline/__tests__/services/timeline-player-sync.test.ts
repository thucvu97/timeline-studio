...
import * as speedRampingUtils from "../../utils/speed-ramping-utils"

// Mock interpolateSpeed function
vi.mock("../../utils/speed-ramping-utils", () => ({
  interpolateSpeed: vi.fn(),
}))
...
describe("TimelinePlayerSync", () => {
  ...
  describe("Speed Ramping Integration", () => {
    beforeEach(() => {
      sync.setPlayerContext(mockPlayerContext)
      vi.resetAllMocks()
    })

    it("должен включать speed ramping для клипа с конфигурацией", () => {
      vi.mocked(speedRampingUtils.interpolateSpeed).mockReturnValue(1.5)
      ...
      // Очищаем моки функций, но сохраняем basePlaybackRate
      vi.mocked(speedRampingUtils.interpolateSpeed).mockClear()
      vi.mocked(mockPlayerContext.setSpeedRampingEnabled).mockClear()
      vi.mocked(mockPlayerContext.updatePlaybackRate).mockClear()

      // Синхронизируем время в середине клипа
      sync.syncPlaybackTime(20) // timeline time = 20, clip relative time = 10

      expect(mockPlayerContext.setSpeedRampingEnabled).toHaveBeenCalledWith(true)
      expect(speedRampingUtils.interpolateSpeed).toHaveBeenCalledWith(clip.speedRamping!.keyframes, 10)
      expect(mockPlayerContext.updatePlaybackRate).toHaveBeenCalledWith(1.5)
    })

    it("должен отключать speed ramping для клипа без конфигурации", () => {
      // Сначала устанавливаем speedRampingEnabled в true
      mockPlayerContext.speedRampingEnabled = true
      ...
      expect(mockPlayerContext.setSpeedRampingEnabled).toHaveBeenCalledWith(false)
      expect(mockPlayerContext.updatePlaybackRate).toHaveBeenCalledWith(1.0)
    })

    it("должен использовать базовую скорость для вычисления финальной скорости", () => {
      vi.mocked(speedRampingUtils.interpolateSpeed).mockReturnValue(2.0)

      mockPlayerContext.basePlaybackRate = 1.5
      ...
      const clip = createClip({
        ...
        speedRamping: {
          ...
          keyframes: [
            { id: "1", time: 0, value: 1.0, interpolation: "linear" },
            { id: "2", time: 10, value: 3.0, interpolation: "linear" },
          ],
          maintainPitch: true,
          minSpeed: 0.1,
          maxSpeed: 4.0,
      ...
      sync.syncPlaybackTime(5)

      expect(mockPlayerContext.updatePlaybackRate).toHaveBeenCalledWith(3.0) // 1.5 * 2.0
    })

    it("не должен вызывать speed ramping методы для клипа с disabled конфигурацией", () => {
      const clip = createClip({
        startTime: 0,
        duration: 10,
        speedRamping: {
          enabled: false,
          keyframes: [{ id: "1", time: 0, value: 2.0, interpolation: "linear" }],
          maintainPitch: true,
          minSpeed: 0.1,
          maxSpeed: 4.0,
          showGraph: true,
      ...
      expect(mockPlayerContext.setSpeedRampingEnabled).toHaveBeenCalledWith(false)
      expect(speedRampingUtils.interpolateSpeed).not.toHaveBeenCalled()
    })

    it("не должен обновлять speed ramping вне границ клипа", () => {
      const clip = createClip({
        startTime: 10,
        duration: 10,
        speedRamping: {
      ...
      sync.syncPlaybackTime(25) // После конца клипа

      expect(mockPlayerContext.setSpeedRampingEnabled).not.toHaveBeenCalled()
      expect(speedRampingUtils.interpolateSpeed).not.toHaveBeenCalled()
    })

    it("должен включать speed ramping только один раз", () => {
      vi.mocked(speedRampingUtils.interpolateSpeed).mockReturnValue(1.5)
      ...
      const clip = createClip({
        ...
        speedRamping: {
          enabled: true,
          keyframes: [{ id: "1", time: 0, value: 1.0, interpolation: "linear" }],
          maintainPitch: true,
          minSpeed: 0.1,
          maxSpeed: 4.0,
          showGraph: true,
      ...
      // Убеждаемся, что speed ramping изначально выключен
      mockPlayerContext.speedRampingEnabled = false
      mockPlayerContext.basePlaybackRate = 1.0
      vi.mocked(mockPlayerContext.updatePlaybackRate).mockClear()

      // Первый вызов - включаем speed ramping
      sync.syncPlaybackTime(5)
      expect(mockPlayerContext.setSpeedRampingEnabled).toHaveBeenCalledWith(true)

      // Устанавливаем флаг как включенный и очищаем моки
      mockPlayerContext.speedRampingEnabled = true
      vi.mocked(mockPlayerContext.setSpeedRampingEnabled).mockClear()

      // Второй вызов - не должны снова включать
      sync.syncPlaybackTime(6)
      expect(mockPlayerContext.setSpeedRampingEnabled).not.toHaveBeenCalled()
      expect(mockPlayerContext.updatePlaybackRate).toHaveBeenCalledWith(1.5)
    })
  })
