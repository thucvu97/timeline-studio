...
describe("TimelineMachine", () => {
  ...
  describe("Clip operations", () => {
    ...
    it("должен разделить клип", () => {
      ...
      const clip =
        actor.getSnapshot().context.project?.globalTracks[0].clips[0];
  ...
  describe("Track operations additional tests", () => {
    ...
    it("должен изменить порядок треков", () => {
      ...
      const trackIds =
        actor
          .getSnapshot()
          .context.project?.globalTracks.map((t: any) => t.id) || [];
  ...
  describe("J-Cut / L-Cut operations", () => {
    ...
    it("должен связать клипы", () => {
      ...
      const audioTrackId =
        actor.getSnapshot().context.project?.globalTracks[1].id;
      ...
      const audioClipId =
        actor.getSnapshot().context.project?.globalTracks[1].clips[0].id;
    ...
    it("должен разъединить клипы", () => {
      ...
      let videoClip =
        actor.getSnapshot().context.project?.globalTracks[0].clips[0];
      let audioClip =
        actor.getSnapshot().context.project?.globalTracks[1].clips[0];
      expect(videoClip?.isLinked).toBe(true);
      expect(audioClip?.isLinked).toBe(true);
  ...
  describe("Speed Ramping operations", () => {
    ...
    it("должен включить speed ramping", () => {
      ...
      expect(snapshot.context.speedRampingConfigs[clipId].maintainPitch).toBe(
        true,
      );
    ...
    it("должен обновить speed keyframe", () => {
      ...
      const keyframeId =
        actor.getSnapshot().context.speedRampingConfigs[clipId].keyframes[0].id;
      ...
      const updatedKeyframe =
        snapshot.context.speedRampingConfigs[clipId].keyframes[0];
    ...
    it("должен удалить speed keyframe", () => {
      ...
      const snapshot = actor.getSnapshot();
      expect(
        snapshot.context.speedRampingConfigs[clipId].keyframes,
      ).toHaveLength(0);
