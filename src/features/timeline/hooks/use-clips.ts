...
export function useClips(): UseClipsReturn {
  ...
  const findClip = useMemo(() => (clipId: string) => {
    if (!project) return null
    return findClipById(project, clipId)
  }, [project])

  const getClipsByTrack = useMemo(() => (trackId: string) => {
    return clips.filter(clip => clip.trackId === trackId)
  }, [clips])

  const getClipsInRange = useMemo(() => (startTime: number, endTime: number) => {
    if (!project) return []
    return getClipsInTimeRange(project, startTime, endTime)
  }, [project])

  const getClipsByType = useMemo(() => (trackType: TrackType) => {
    if (!project) return []
  ...
  const findNearestClipToTime = useMemo(() => (time: number, trackType?: TrackType) => {
    if (!project) return null
