...
export function useSubtitleStyles(): UseSubtitleStylesReturn {
  ...
  const getStyleById = useCallback((styleId: string): SubtitleStyle | undefined => {
    return subtitleStyles.find(style => style.id === styleId)
  }, [subtitleStyles])
  
  const getStyleByName = useCallback((name: string): SubtitleStyle | undefined => {
    return subtitleStyles.find(style => style.name === name)
  }, [subtitleStyles])
  
  const addSubtitleStyle = useCallback((style: SubtitleStyle) => {
    if (!project) return
  ...
  const updateSubtitleStyle = useCallback((styleId: string, updates: Partial<SubtitleStyle>) => {
    if (!project) return
  ...
  const removeSubtitleStyle = useCallback((styleId: string) => {
    if (!project) return
  ...
  const getDefaultStyle = useCallback((): SubtitleStyle | undefined => {
    ...
    return subtitleStyles.find(style => 
      style.isBuiltIn && (style.name === 'Классический' || style.name === 'Default')
    ) || subtitleStyles.find(style => style.isBuiltIn) || subtitleStyles[0]
  }, [subtitleStyles])
