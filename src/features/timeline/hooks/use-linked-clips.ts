...
export function useLinkedClips(): UseLinkedClipsReturn {
  ...
  const linkClips = useCallback((clip1Id: string, clip2Id: string): boolean => {
    const clips = getAllClips()
  ...
  const unlinkClips = useCallback((clip1Id: string, clip2Id: string): boolean => {
    send({
      type: 'UPDATE_CLIP',
      clipId: clip1Id,
      updates: {
  ...
  const getLinkedClip = useCallback((clipId: string): TimelineClip | null => {
    const clips = getAllClips()
    const clip = clips.find(c => c.id === clipId)
  ...
  const isClipLinked = useCallback((clipId: string): boolean => {
    const clips = getAllClips()
    const clip = clips.find(c => c.id === clipId)
    return Boolean(clip?.linkedClipId)
  }, [getAllClips])

  const getLinkType = useCallback((clipId: string): 'video-audio' | 'audio-video' | 'multi-camera' | null => {
    const clips = getAllClips()
    const clip = clips.find(c => c.id === clipId)
    ...
    if (clip.mediaFile?.type === 'video' && linkedClip.mediaFile?.type === 'audio') {
      return 'video-audio'
    } else if (clip.mediaFile?.type === 'audio' && linkedClip.mediaFile?.type === 'video') {
      return 'audio-video'
    } else if (clip.mediaFile?.type === 'video' && linkedClip.mediaFile?.type === 'video') {
      return 'multi-camera'
    }
  ...
  const autoLinkClipsByMedia = useCallback((mediaFileId: string): number => {
    const clips = getAllClips()
  ...
  const syncLinkedClips = useCallback((clipId: string, updates: Partial<TimelineClip>) => {
  ...
  const validateLinkSync = useCallback((clipId: string): boolean => {
    const clips = getAllClips()
    const clip = clips.find(c => c.id === clipId)
    const linkedClip = getLinkedClip(clipId)
    ...
    return (
      ...
      clip.duration === linkedClip.duration
    )
  }, [getAllClips, getLinkedClip])

  const moveLinkedClips = useCallback((clipId: string, newStartTime: number) => {
    send({
      type: 'UPDATE_CLIP',
      clipId,
  ...
  const resizeLinkedClips = useCallback((clipId: string, newDuration: number) => {
    send({
      type: 'UPDATE_CLIP',
      clipId,
  ...
  const createLinkedGroup = useCallback((clipIds: string[]): boolean => {
  ...
  const breakLinkedGroup = useCallback((clipId: string): boolean => {
    const clips = getAllClips()
    const clip = clips.find(c => c.id === clipId)
