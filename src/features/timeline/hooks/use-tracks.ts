...
export function useTracks(): UseTracksReturn {
  ...
  const sectionTracks = useMemo(() => {
    if (!project) return [];
    const allSectionTracks = project.sections.flatMap(
  ...
  const visibleTracks = useMemo(() => {
    return tracks.filter(
      (track) =>
        !track.isHidden && uiState.visibleTrackTypes.includes(track.type),
    );
  ...
  const canAddTrackToSection = (
    ...
    if (
      trackType === "video" &&
      existingTypes.filter((t) => t === "video").length >= 10
    ) {
      return false;
    }
