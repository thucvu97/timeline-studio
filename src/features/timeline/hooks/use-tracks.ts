...
export function useTracks(): UseTracksReturn {
  ...
  const visibleTracks = useMemo(() => {
    return tracks.filter(track =>
      !track.isHidden &&
      uiState.visibleTrackTypes.includes(track.type)
    )
  ...
  const getTracksByTypeFunc = useMemo(() => (type: TrackType) => {
    if (!project) return []
    return getTracksByType(project, type)
  }, [project])

  const getTracksBySection = useMemo(() => (sectionId: string) => {
    if (!project) return []
    const section = project.sections.find(s => s.id === sectionId)
    return section ? sortTracksByOrder(section.tracks) : []
  }, [project])

  const findTrack = useMemo(() => (trackId: string) => {
