...
export function useSplitEdit(): UseSplitEditReturn {
  ...
  const toggleSplitEdit = useCallback((enabled: boolean) => {
    service.updateConfig({ enabled })
    setConfig(service.getConfig())
  }, [service])

  const setTool = useCallback((tool: 'razor' | 'select' | 'slip' | 'slide') => {
    service.updateConfig({ tool })
    setConfig(service.getConfig())
  }, [service])

  const updateToolSettings = useCallback((settings: Partial<SplitEditToolSettings>) => {
    service.updateToolSettings(settings)
    setToolSettings(service.getToolSettings())
  }, [service])

  const createLCut = useCallback(async (
    videoClipId: string,
    audioClipId: string,
    position: number
  ): Promise<boolean> => {
  ...
  const createJCut = useCallback(async (
    videoClipId: string,
    audioClipId: string,
    position: number
  ): Promise<boolean> => {
  ...
  const splitAtPlayhead = useCallback(async (clipId: string, position: number): Promise<boolean> => {
    const clips = getAllClips()
    ...
    if (result.success && result.updatedClips) {
      ...
      for (const clip of result.updatedClips) {
        if (clip.id !== clipId) { // Добавляем только новые клипы
          send({
            type: 'ADD_CLIP',
  ...
  const removeSplitEdit = useCallback((splitEditId: string): boolean => {
  ...
  const getSplitEditsForClip = useCallback((clipId: string): SplitEdit[] => {
  ...
  const showPreview = useCallback((position: number, type: 'L-cut' | 'J-cut' | 'split-at-playhead') => {
  ...
  const updatePreviewPosition = useCallback((position: number) => {
    service.updatePreviewPosition(position)
    setVisualSettings(service.getVisualSettings())
  }, [service])

  const getLinkedClips = useCallback((clipId: string) => {
    const clips = getAllClips()
  ...
  const performOperation = useCallback(async (operation: SplitEditOperation): Promise<boolean> => {
    const clips = getAllClips()
  ...
  const canPerformOperation = useCallback((operation: SplitEditOperation): boolean => {
    const clips = getAllClips()
  ...
  const validateSplitEdit = useCallback((splitEdit: SplitEdit): boolean => {
    const clips = getAllClips()
