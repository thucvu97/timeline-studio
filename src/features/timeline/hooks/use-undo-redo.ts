/**
 * Hook для работы с расширенной Undo/Redo системой
 */
    ...
    async (action: UndoRedoAction, isUndo: boolean): Promise<boolean> => {
      try {
        ...
        switch (action.type) {
          case "ADD_CLIP":
            if (isUndo) {
              await timeline.removeClip(actionData.clipId)
            } else {
              await timeline.addClip(actionData.trackId, actionData.mediaFile, actionData.time)
            }
            break
          ...
          case "REMOVE_CLIP":
            if (isUndo) {
              ...
              if (actionData.clipProperties) {
                await timeline.updateClip(actionData.clipId, actionData.clipProperties)
              }
            } else {
              await timeline.removeClip(actionData.clipId)
            }
          ...
          case "MOVE_CLIP":
            if (isUndo) {
              await timeline.moveClip(actionData.clipId, actionData.oldTrackId, actionData.oldTime)
            } else {
              await timeline.moveClip(actionData.clipId, actionData.newTrackId, actionData.newTime)
            }
            break

          case "TRIM_CLIP":
            if (isUndo) {
              await timeline.trimClip(actionData.clipId, actionData.oldStartTime, actionData.oldEndTime)
            } else {
              await timeline.trimClip(actionData.clipId, actionData.newStartTime, actionData.newEndTime)
            }
            break

          case "UPDATE_CLIP":
            if (isUndo) {
              await timeline.updateClip(actionData.clipId, actionData.oldProperties)
            } else {
              await timeline.updateClip(actionData.clipId, actionData.newProperties)
            }
            break

          case "ADD_TRACK":
            if (isUndo) {
              await timeline.removeTrack(actionData.trackId)
            } else {
              await timeline.addTrack(actionData.trackType, actionData.trackName)
            }
            break
          ...
          case "REMOVE_TRACK":
            if (isUndo) {
              ...
              for (const clip of actionData.clips || []) {
                await timeline.addClip(actionData.trackId, clip.mediaFile, clip.startTime)
              }
            } else {
              await timeline.removeTrack(actionData.trackId)
            }
          ...
          case "UPDATE_TRACK":
            if (isUndo) {
              await timeline.updateTrack(actionData.trackId, actionData.oldProperties)
            } else {
              await timeline.updateTrack(actionData.trackId, actionData.newProperties)
            }
            break
          ...
          case "BATCH_OPERATION":
            if (isUndo) {
              ...
              if (timeline.batchUpdateClips && actionData.originalClips) {
                await timeline.batchUpdateClips(actionData.originalClips)
              }
            } else {
              // Применяем batch операцию
              if (timeline.batchUpdateClips && actionData.updatedClips) {
                await timeline.batchUpdateClips(actionData.updatedClips)
              }
            }
            break
...
export const UndoRedoHelpers = {
  ...
  createRemoveClipAction: (clip: TimelineClip) => ({
    type: "REMOVE_CLIP" as ActionType,
    description: `Удалить клип "${clip.name}"`,
    undoData: {
      clipId: clip.id,
  ...
  createBatchOperationAction: (description: string, originalClips: TimelineClip[], updatedClips: TimelineClip[]) => ({
