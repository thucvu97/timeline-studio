...
export function useClipGroups(): UseClipGroupsReturn {
  ...
  // Разгруппировка
  const ungroupClips = useCallback((groupId: GroupId): GroupOperationResult => {
  ...
  const toggleCollapse = useCallback((groupId: GroupId) => {
  ...
  // Блокировка группы
  const lockGroup = useCallback((groupId: GroupId, locked: boolean) => {
  ...
  const setGroupColor = useCallback((groupId: GroupId, color: string) => {
  ...
  const updateNestedSequence = useCallback((
    sequenceId: GroupId, 
    updates: Partial<NestedSequence>
  ) => {
    groupManager.updateNestedSequence(sequenceId, updates);
  }, [groupManager]);

  // Разбор вложенной последовательности
  const breakApartSequence = useCallback((sequenceId: GroupId): GroupOperationResult => {
  ...
  const getGroupByClip = useCallback((clipId: string) => {
    return groupManager.getGroupByClip(clipId);
  }, [groupManager]);

  // Проверка, находится ли клип в группе
  const isClipInGroup = useCallback((clipId: string) => {
    return groupManager.isClipInGroup(clipId);
  }, [groupManager]);

  // Получение клипов в группе
  const getClipsInGroup = useCallback((groupId: GroupId): TimelineClip[] => {
