...
export function TimelineProvider({ children, services }: TimelineProviderProps) {
  const [state, send] = useMachine(timelineMachine, {
    services: {
      saveProjectService: services?.saveProjectService || defaultSaveProjectService,
    }
  })
  ...
  // Проект
  const createProject = useCallback((name: string, settings?: any) => {
    send({ type: "CREATE_PROJECT", name, settings })
  }, [send])

  const loadProject = useCallback((project: TimelineProject) => {
  ...
  // Секции
  const addSection = useCallback((name: string, startTime: number, duration: number, realStartTime?: Date) => {
  ...
  const updateSection = useCallback((sectionId: string, updates: Partial<TimelineSection>) => {
    send({ type: "UPDATE_SECTION", sectionId, updates })
  }, [send])

  // Треки
  const addTrack = useCallback((trackType: TrackType, sectionId?: string, name?: string) => {
  ...
  // Клипы
  const addClip = useCallback((trackId: string, mediaFile: MediaFile, startTime: number, duration?: number) => {
  ...
  const selectSections = useCallback((sectionIds: string[], addToSelection = false) => {
  ...
  const paste = useCallback((targetTrackId?: string, targetTime?: number) => {
