...
export function pasteClips(
  ...
  const clipsToInsert = clipboardData.clips.map(clip => {
    ...
    newClip.effects = newClip.effects?.map(effect => ({
      ...effect,
      id: `paste-${effect.id}-${Date.now()}`,
    })) || []
    
    newClip.filters = newClip.filters?.map(filter => ({
      ...filter,
      id: `paste-${filter.id}-${Date.now()}`,
    })) || []
    
    newClip.transitions = newClip.transitions?.map(transition => ({
      ...transition,
 ...
 */
function insertClipsWithRipple(
  project: TimelineProject,
  clipsToInsert: TimelineClip[]
): TimelineProject {
...
export function splitClip(
  ...
  if (createNewIds) {
    rightClip.effects = rightClip.effects?.map(effect => ({
      ...effect,
      id: `${effect.id}-split`,
    })) || []
    
    rightClip.filters = rightClip.filters?.map(filter => ({
      ...filter,
      id: `${filter.id}-split`,
    })) || []
    
    rightClip.transitions = rightClip.transitions?.map(transition => ({
      ...transition,
