...
export function ProjectSettingsProviderV2({ children }: ProjectSettingsProviderV2Props) {
  ...
  // Действия
  const updateSettings = useCallback(async (newSettings: Partial<ProjectSettings>) => {
  ...
  return (
    <ProjectSettingsContextV2.Provider value={contextValue}>
      {children}
    </ProjectSettingsContextV2.Provider>
  )
}
