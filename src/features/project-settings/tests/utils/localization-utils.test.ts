...
describe('localization-utils', () => {
  describe('getAspectRatioLabel', () => {
    ...
    it('должен возвращать локализованные метки для известных соотношений', () => {
      ...
      expect(getAspectRatioLabel('Социальные сети', mockT)).toBe('Social Media')
    ...
    it('должен возвращать исходную метку для неизвестных соотношений', () => {
      expect(getAspectRatioLabel('Неизвестное соотношение', mockT)).toBe('Неизвестное соотношение')
    ...
    it('должен вызывать функцию перевода для каждой известной метки', () => {
      ...
      expect(mockT).toHaveBeenCalledWith('dialogs.projectSettings.aspectRatioLabels.widescreen')
      expect(mockT).toHaveBeenCalledWith('dialogs.projectSettings.aspectRatioLabels.portrait')
      expect(mockT).toHaveBeenCalledWith('dialogs.projectSettings.aspectRatioLabels.social')
      expect(mockT).toHaveBeenCalledWith('dialogs.projectSettings.aspectRatioLabels.standard')
      expect(mockT).toHaveBeenCalledWith('dialogs.projectSettings.aspectRatioLabels.vertical')
      expect(mockT).toHaveBeenCalledWith('dialogs.projectSettings.aspectRatioLabels.cinema')
    })
  })
  ...
  describe('getLockedAspectRatioText', () => {
    ...
    it('должен передавать соотношение сторон в опциях', () => {
      ...
      expect(mockT).toHaveBeenCalledWith('dialogs.projectSettings.aspectRatioLocked', { ratio: '21:9' })
    })
  })
  ...
  describe('getUnlockedAspectRatioText', () => {
    ...
    it('должен возвращать текст для разблокированного соотношения сторон', () => {
      expect(getUnlockedAspectRatioText('16:9', mockT)).toBe('Unlocked from 16:9')
      expect(getUnlockedAspectRatioText('1:1', mockT)).toBe('Unlocked from 1:1')
      expect(getUnlockedAspectRatioText('4:3', mockT)).toBe('Unlocked from 4:3')
    })
    ...
    it('должен передавать соотношение сторон в опциях', () => {
      ...
      expect(mockT).toHaveBeenCalledWith('dialogs.projectSettings.aspectRatioUnlocked', { ratio: '9:16' })
    })
  })
  ...
  describe('getCustomAspectRatioText', () => {
    ...
    it('должен вызывать функцию перевода с правильным ключом', () => {
      ...
      expect(mockT).toHaveBeenCalledWith('dialogs.projectSettings.aspectRatioLabels.custom')
    })
  })
  ...
  describe('интеграционные тесты', () => {
    it('должен корректно работать с реальной функцией перевода', () => {
      const realT = (key: string, options?: any) => {
        const translations: Record<string, string> = {
          'dialogs.projectSettings.aspectRatioLabels.widescreen': 'Широкоэкранный',
          'dialogs.projectSettings.aspectRatioLabels.custom': 'Пользовательский',
      ...
      expect(getAspectRatioLabel('Широкоэкнранный', realT)).toBe('Широкоэкранный')
      expect(getCustomAspectRatioText(realT)).toBe('Пользовательский')
      expect(getLockedAspectRatioText('16:9', realT)).toBe('Заблокировано на 16:9')
      expect(getUnlockedAspectRatioText('16:9', realT)).toBe('Разблокировано с 16:9')
    })
  })
})
