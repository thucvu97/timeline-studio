...
/**
 * Хук для получения локализованного названия шаблона по id
 * @param id Идентификатор шаблона
 * @returns Локализованное название шаблона
 */
export function useTemplateLabel(id: string): string {
  const { t } = useTranslation()
  return t(`templates.templateLabels.${id}`)
}

/**
 * Хук для получения локализованного описания шаблона по id
 * @param id Идентификатор шаблона
 * @returns Локализованное описание шаблона
 */
export function useTemplateDescription(id: string): string {
  const { t } = useTranslation()
  return t(`templates.templateDescriptions.${id}`)
}
