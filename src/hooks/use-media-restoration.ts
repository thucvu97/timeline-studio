...
export function useMediaRestoration() {
   ...
   */
  const restoreProjectMedia = useCallback(async (
   ...
   */
  const handleMissingFilesResolution = useCallback(async (
    resolved: Array<{ file: SavedMediaFile; newPath?: string; action: 'found' | 'remove' }>
  ): Promise<{
    ...
    for (const resolution of resolved) {
      if (resolution.action === 'found' && resolution.newPath) {
        ...
        const restoredFile: MediaFile = {
          ...
          probeData: resolution.file.metadata.probeData || { streams: [], format: {} },
